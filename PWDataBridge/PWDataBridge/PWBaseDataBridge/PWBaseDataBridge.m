...
- (void)addBridgeObserver:(NSObject *)observer forKeyPath:(NSString *)keyPath correction:(PWBaseDataBridgeBeforeReturnBlock)correction action:(SEL)action{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        if (!model) {
            ...
            [self->models setObject:model forKey:key];
            if (![self->addKeyPaths containsObject:keyPath] && ([self->propertys containsObject:keyPath] || [self->propertys containsObject:[NSString stringWithFormat:@"_%@" , keyPath]])) {
                [self->addKeyPaths addObject:keyPath];
        ...
        model.block = nil;
        model.removed = NO;
        [self->removeKeys removeObject:key];
        dispatch_semaphore_signal(self->semaphore);
    });
}
...
//    });
        ...
        if ([self->addKeyPaths containsObject:keyPath]) {
            if (keyPath) {
                [self removeObserver:self forKeyPath:keyPath];
                [self->addKeyPaths removeObject:keyPath];
            }
        }
        dispatch_semaphore_signal(self->semaphore);
        //    [lock unlock];
...
//    });
        ...
        NSArray *array = [NSArray arrayWithArray:self->addKeyPaths];
...
//    [lock lock];
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        dispatch_semaphore_wait(self->semaphore, DISPATCH_TIME_FOREVER);
        NSArray *array = [NSArray arrayWithArray:self->addKeyPaths];
        NSArray *keys = [NSArray arrayWithArray:[self->models allKeys]];
...
//    [lock lock];
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        @autoreleasepool{
            ...
            NSArray *array = [NSArray arrayWithArray:self->removeKeys];
            [array enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
